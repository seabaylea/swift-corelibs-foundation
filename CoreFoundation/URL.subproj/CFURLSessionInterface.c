...
/*
 import Foundation
 let url = NSURL(fileURLWithPath: "/Applications/Xcode-7.3.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX10.11.sdk/usr/include/curl/curl.h")
 let header = try! NSString(contentsOfURL: url, encoding: NSUTF8StringEncoding)
 let scanner = NSScanner(string: header as String)
 scanner.charactersToBeSkipped = NSCharacterSet.illegalCharacterSet()
 let valid = NSMutableCharacterSet.alphanumericCharacterSet()
 valid.addCharactersInString("_")
 // Scan all lines like this:
 //   CURLM_OK,
 while !scanner.atEnd {
 let start = "\n  CURLE_"
 let space = " "
 var name: NSString? = nil
 if scanner.scanUpToString(start, intoString: nil) &&
 scanner.scanString(start, intoString: nil) &&
 scanner.scanCharactersFromSet(valid, intoString: &name) {
 if let n = name {
 print("CFURLSessionEasyCode const CFURLSessionEasyCode\(n) = { CURLE_\(n) };")
 }
 }
 }
 */
...
/*
 import Foundation
 let url = NSURL(fileURLWithPath: "/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX10.11.sdk/usr/include/curl/curl.h")
 let header = try! NSString(contentsOfURL: url, encoding: NSUTF8StringEncoding)
 let scanner = NSScanner(string: header as String)
 let valid = NSMutableCharacterSet.alphanumericCharacterSet()
 valid.addCharactersInString("_")
 ...
 scanner.scanCharactersFromSet(valid, intoString: &name) {
 if let n = name {
 print("CFURLSessionProtocol const CFURLSessionProtocol\(n) = CURLPROTO_\(n);")
 }
 }
 }
 */
...
/*
 ...
 let header = try! NSString(contentsOfURL: url, encoding: NSUTF8StringEncoding)
 let scanner = NSScanner(string: header as String)
 let skip = "The first argument in the CINIT() macro is the [what]"
 guard scanner.scanUpToString(skip, intoString: nil) &&
 scanner.scanString(skip, intoString: nil) else {
 fatalError("Could not find marker '\(skip)'")
 }
 // Scan all lines like this:
 ...
 if scanner.scanUpToString(start, intoString: nil) &&
 scanner.scanString(start, intoString: nil) &&
 scanner.scanUpToString(comma, intoString: &name) {
 if let n = name {
 print("CFURLSessionOption const CFURLSessionOption\(n) = { CURLOPT_\(n) };")
 }
 }
 }
 */
...
/*
 ...
 scanner.scanCharactersFromSet(valid, intoString: &name) {
 if let n = name {
 print("CFURLSessionInfo const CFURLSessionInfo\(n) = { CURLINFO_\(n) };")
 }
 }
 }
 */
...
/*
 import Foundation
 let url = NSURL(fileURLWithPath: "/Applications/Xcode-7.3.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX10.11.sdk/usr/include/curl/multi.h")
 let header = try! NSString(contentsOfURL: url, encoding: NSUTF8StringEncoding)
 let scanner = NSScanner(string: header as String)
 let skip = "This is the socket callback function pointer"
 guard scanner.scanUpToString(skip, intoString: nil) &&
 scanner.scanString(skip, intoString: nil) else {
 fatalError("Could not find marker '\(skip)'")
 }
 ...
 scanner.scanUpToString(comma, intoString: &name) {
 if let n = name {
 print("CFURLSessionMultiOption const CFURLSessionMultiOption\(n) = { CURLMOPT_\(n) };")
 }
 }
 }
 */
...
/*
 ...
 scanner.scanCharactersFromSet(valid, intoString: &name) {
 if let n = name {
 print("CFURLSessionMultiCode const CFURLSessionMultiCode\(n) = { CURLM_\(n) };")
 }
 }
 }
 */
