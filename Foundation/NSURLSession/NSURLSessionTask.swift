...
public class NSURLSessionTask : NSObject, NSCopying {
    ...
    private var totalDownloaded = 0
    private unowned let session: NSURLSessionProtocol
    private let body: Body
    private let tempFileURL: NSURL
    ...
    public override init() {
        ...
        NSFileManager.defaultManager().createFile(atPath: fileName, contents: nil)
        self.tempFileURL = NSURL(fileURLWithPath: fileName)
        super.init()
    }
...
private extension NSURLSessionTask.Body {
    ...
    func getBodyLength() throws -> UInt64? {
        ...
        case .file(let fileURL):
            guard let s = try NSFileManager.defaultManager().attributesOfItem(atPath: fileURL.path!)[NSFileSize] as? NSNumber else {
...
extension NSURLSessionTask: EasyHandleDelegate {
    ...
    private func notifyDelegate(aboutReceivedData data: UnsafeBufferPointer<Int8>) {
        if case .taskDelegate(let delegate) = session.behaviour(for: self),
            ...
            let downloadDelegate = delegate as? NSURLSessionDownloadDelegate,
            let task = self as? NSURLSessionDownloadTask {

                guard let s = session as? NSURLSession else { fatalError() }
                let d = NSData(bytes: UnsafeMutablePointer<Void>(data.baseAddress), length: data.count)
                let fileHandle = try! NSFileHandle(forWritingToURL: tempFileURL)
                fileHandle.seekToEndOfFile()
                fileHandle.writeData(d)
                self.totalDownloaded += data.count
            
                s.delegateQueue.addOperationWithBlock {
                    downloadDelegate.urlSession(session: s, downloadTask: task, didWriteData: Int64(data.count), totalBytesWritten: Int64(self.totalDownloaded),
                        totalBytesExpectedToWrite: Int64(self.easyHandle.fileLength))
                }
                if Int(self.easyHandle.fileLength) == totalDownloaded {
                    fileHandle.closeFile()
                    s.delegateQueue.addOperationWithBlock {
                        downloadDelegate.urlSession(session: s, downloadTask: task, didFinishDownloadingTo: self.tempFileURL)
                    }
                }
