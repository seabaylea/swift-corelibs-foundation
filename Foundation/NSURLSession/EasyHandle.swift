...
extension NSURLSessionTask {
    ...
    ///
    /// A single `NSURLSessionTask` may do multiple, sonecutive transfers, and
    ...
    /// `dispatch_queue_t` only -- it is intentionally **not** thread safe.
    internal final class EasyHandle {
        let rawHandle = CFURLSessionEasyHandleInit()
        unowned let delegate: EasyHandleDelegate
        private var headerList: CurlStringList?
        private var pauseState: PauseState = []
        internal var fileLength: Int64 = 0
        init(delegate: EasyHandleDelegate) {
...
extension NSURLSessionTask.EasyHandle: Equatable {}
internal func ==(lhs: NSURLSessionTask.EasyHandle, rhs: NSURLSessionTask.EasyHandle) -> Bool {
...
/*
 /// Private *Pause* management
 private extension NSURLSessionTask.EasyHandle {
 enum PausedState {
 /// The easy handle is paused, either by returning *pause* from one of
 /// the read or write callbacks, or by explicitly calling `pause()`.
 case paused
 /// Same as `.paused` but the transfer completed while in this state.
 case pausedAndTransferComplete(CompletedTransfer)
 /// The easy handle is running (unpaused).
 case running
 }
 /// The state of a completed transfer.
 ///
 /// We store this when we get a *transfer complete* callback while the easy
 /// handle is paused, because we can't call the delegate in that state.
 struct CompletedTransfer {
 let errorCode: Int?
 }
 }
 */
internal extension NSURLSessionTask.EasyHandle {
    func completedTransfer(withErrorCode errorCode: Int?) {
        //        switch pausedState {
        //        case .running:
        delegate.transferCompleted(withErrorCode: errorCode)
        //        case .paused:
        //            pausedState = .pausedAndTransferComplete(CompletedTransfer(errorCode: errorCode))
        //        case .pausedAndTransferComplete:
        //            fatalError("Transfer completed a second time while paused. Can only complete once.")
        //        }
    }
}
internal protocol EasyHandleDelegate: class {
    /// Handle data read from the network.
    /// - returns: the action to be taken: abort, proceed, or pause.
    func didReceive(data: UnsafeBufferPointer<Int8>) -> NSURLSessionTask.EasyHandle.Action
    /// Handle header data read from the network.
    /// - returns: the action to be taken: abort, proceed, or pause.
    func didReceive(headerData data: UnsafeBufferPointer<Int8>) -> NSURLSessionTask.EasyHandle.Action
...
extension NSURLSessionTask.EasyHandle {
    ...
    func set(debugOutputOn flag: Bool, task: NSURLSessionTask) {
        if flag {
            try! CFURLSession_easy_setopt_ptr(rawHandle, CFURLSessionOptionDEBUGDATA, UnsafeMutablePointer<Void>(unsafeAddress(of: task))).asError()
    ...
    func set(url: NSURL) {
        url.absoluteString.withCString {
            try! CFURLSession_easy_setopt_ptr(rawHandle, CFURLSessionOptionURL, UnsafeMutablePointer($0)).asError()
        }
    }
    ...
    func set(customHeaders headers: [String]) {
        let list = CurlStringList(headers)
    ...
    func set(requestMethod method: String) {
        method.withCString {
            try! CFURLSession_easy_setopt_ptr(rawHandle, CFURLSessionOptionCUSTOMREQUEST, UnsafeMutablePointer($0)).asError()
        }
    }
...
private func printLibcurlDebug(handle: CFURLSessionEasyHandle, type: CInt, data: UnsafeMutablePointer<Int8>, size: Int, userInfo: UnsafeMutablePointer<Void>?) -> CInt {
    ...
    let text = String(utf8Buffer: buffer) ?? ""
    guard let userInfo = userInfo else { return 0 }
    let task = Unmanaged<NSURLSessionTask>.fromOpaque(userInfo).takeUnretainedValue()
    printLibcurlDebug(type: info, data: text, task: task)
    return 0
}
private func printLibcurlDebug(type: CFURLSessionInfo, data: String, task: NSURLSessionTask) {
...
extension NSURLSessionTask.EasyHandle {
    ...
    struct PauseState : OptionSet {
        let rawValue: Int8
        init(rawValue: Int8) { self.rawValue = rawValue }
        static let receivePaused = PauseState(rawValue: 1 << 0)
        static let sendPaused = PauseState(rawValue: 1 << 1)
    }
}
extension NSURLSessionTask.EasyHandle.PauseState {
    func setState(on handle: NSURLSessionTask.EasyHandle) {
        try! CFURLSessionEasyHandleSetPauseState(handle.rawHandle, contains(.sendPaused) ? 1 : 0, contains(.receivePaused) ? 1 : 0).asError()
    }
}
extension NSURLSessionTask.EasyHandle.PauseState : Streamable {
...
extension NSURLSessionTask.EasyHandle {
    ...
    func pauseReceive() {
        NSURLSession.printDebug("[EasyHandle] pause receive (\(pauseState))")
    ...
    /// - SeeAlso: https://curl.haxx.se/libcurl/c/curl_easy_pause.html
    func unpauseReceive() {
        NSURLSession.printDebug("[EasyHandle] unpause receive (\(pauseState))")
        guard pauseState.contains(.receivePaused) else { return }
    ...
    /// - SeeAlso: https://curl.haxx.se/libcurl/c/curl_easy_pause.html
    func pauseSend() {
        NSURLSession.printDebug("[EasyHandle] pause send (\(pauseState))")
    ...
    /// - SeeAlso: https://curl.haxx.se/libcurl/c/curl_easy_pause.html
    func unpauseSend() {
        NSURLSession.printDebug("[EasyHandle] unpause send (\(pauseState))")
        guard pauseState.contains(.sendPaused) else { return }
...
extension NSURLSessionTask.EasyHandle {
    ...
    var redirectURL: NSURL? {
        ...
        guard let cstring = p.map({ UnsafePointer<UTF8.CodeUnit>($0) }) else { return nil }
        guard let s = String.decodeCString(cstring, as: UTF8.self, repairingInvalidCodeUnits: false)?.0 else { return nil }
        return NSURL(string: s)
    }
}
private extension NSURLSessionTask.EasyHandle {
    static func from(callbackUserData userdata: UnsafeMutablePointer<Void>?) -> NSURLSessionTask.EasyHandle? {
        guard let userdata = userdata else { return nil }
        return Unmanaged<NSURLSessionTask.EasyHandle>.fromOpaque(userdata).takeUnretainedValue()
    }
}
private extension NSURLSessionTask.EasyHandle {
    ...
    func setupCallbacks() {
        // write
        try! CFURLSession_easy_setopt_ptr(rawHandle, CFURLSessionOptionWRITEDATA, UnsafeMutablePointer<Void>(unsafeAddress(of: self))).asError()
        
        try! CFURLSession_easy_setopt_wc(rawHandle, CFURLSessionOptionWRITEFUNCTION) { (data: UnsafeMutablePointer<Int8>, size: Int, nmemb: Int, userdata: UnsafeMutablePointer<Void>?) -> Int in
            guard let handle = NSURLSessionTask.EasyHandle.from(callbackUserData: userdata) else { return 0 }
            return handle.didReceive(data: data, size: size, nmemb: nmemb)
            }.asError()
        
        // read
        try! CFURLSession_easy_setopt_ptr(rawHandle, CFURLSessionOptionREADDATA, UnsafeMutablePointer<Void>(unsafeAddress(of: self))).asError()
        try! CFURLSession_easy_setopt_wc(rawHandle, CFURLSessionOptionREADFUNCTION) { (data: UnsafeMutablePointer<Int8>, size: Int, nmemb: Int, userdata: UnsafeMutablePointer<Void>?) -> Int in
            guard let handle = NSURLSessionTask.EasyHandle.from(callbackUserData: userdata) else { return 0 }
            return handle.fill(writeBuffer: data, size: size, nmemb: nmemb)
            }.asError()
         
        // header
        try! CFURLSession_easy_setopt_ptr(rawHandle, CFURLSessionOptionHEADERDATA, UnsafeMutablePointer<Void>(unsafeAddress(of: self))).asError()
        try! CFURLSession_easy_setopt_wc(rawHandle, CFURLSessionOptionHEADERFUNCTION) { (data: UnsafeMutablePointer<Int8>, size: Int, nmemb: Int, userdata: UnsafeMutablePointer<Void>?) -> Int in
            guard let handle = NSURLSessionTask.EasyHandle.from(callbackUserData: userdata) else { return 0 }
            var length = Double()
        ...
        // socket options
        try! CFURLSession_easy_setopt_ptr(rawHandle, CFURLSessionOptionSOCKOPTDATA, UnsafeMutablePointer<Void>(unsafeAddress(of: self))).asError()
        try! CFURLSession_easy_setopt_sc(rawHandle, CFURLSessionOptionSOCKOPTFUNCTION) { (userdata: UnsafeMutablePointer<Void>?, fd: CInt, type: CFURLSessionSocketType) -> CInt in
            guard let handle = NSURLSessionTask.EasyHandle.from(callbackUserData: userdata) else { return 0 }
        ...
        try! CFURLSession_easy_setopt_ptr(rawHandle, CFURLSessionOptionSEEKDATA, UnsafeMutablePointer<Void>(unsafeAddress(of: self))).asError()
        try! CFURLSession_easy_setopt_seek(rawHandle, CFURLSessionOptionSEEKFUNCTION, { (userdata, offset, origin) -> Int32 in
            guard let handle = NSURLSessionTask.EasyHandle.from(callbackUserData: userdata) else { return CFURLSessionSeekFail }
        ...
        try! CFURLSession_easy_setopt_ptr(rawHandle, CFURLSessionOptionPROGRESSDATA, UnsafeMutablePointer<Void>(unsafeAddress(of: self))).asError()
        
        try! CFURLSession_easy_setopt_tc(rawHandle, CFURLSessionOptionXFERINFOFUNCTION, { (userdata: UnsafeMutablePointer<Void>?, dltotal :Int64, dlnow: Int64, ultotal: Int64, ulnow: Int64) -> Int32 in
            guard let handle = NSURLSessionTask.EasyHandle.from(callbackUserData: userdata) else { return -1 }
            handle.updateProgressMeter(with: Progress(totalBytesSent: ulnow, totalBytesExpectedToSend: ultotal, totalBytesReceived: dlnow, totalBytesExpectedToReceive: dltotal))
            return 0
        }).asError()
    ...
    func didReceive(data: UnsafeMutablePointer<Int8>, size: Int, nmemb: Int) -> Int {
        NSURLSession.printDebug("[EasyHandle] -> write callback \(size * nmemb)")
        let d: Int = {
            let buffer = UnsafeBufferPointer(start: data, count: size * nmemb)
            switch delegate.didReceive(data: buffer) {
            case .proceed: return size * nmemb
            case .abort: return 0
            case .pause:
                NSURLSession.printDebug("[EasyHandle] pausing receive from callback (\(pauseState))")
                pauseState.insert(.receivePaused)
                return Int(CFURLSessionWriteFuncPause)
            }
        }()
        NSURLSession.printDebug("[EasyHandle] <- write callback \(d)")
        return d
    }
    ...
    func didReceive(headerData data: UnsafeMutablePointer<Int8>, size: Int, nmemb: Int, fileLength: Double) -> Int {
        NSURLSession.printDebug("[EasyHandle] -> header callback \(size * nmemb)")
        ...
        NSURLSession.printDebug("[EasyHandle] <- header callback \(d)")
        return d
    }
    ...
    func fill(writeBuffer data: UnsafeMutablePointer<Int8>, size: Int, nmemb: Int) -> Int {
        NSURLSession.printDebug("[EasyHandle] -> read callback \(size * nmemb)")
        let d: Int = {
            let buffer = UnsafeMutableBufferPointer(start: data, count: size * nmemb)
            switch delegate.fill(writeBuffer: buffer) {
            case .pause:
                NSURLSession.printDebug("[EasyHandle] pausing send from callback (\(pauseState))")
                pauseState.insert(.sendPaused)
        ...
        NSURLSession.printDebug("[EasyHandle] <- read callback \(d)")
        return d
    }
    
    func setSocketOptions(for fd: CInt) throws {
        NSURLSession.printDebug("[EasyHandle] -- socket options callback \(fd)")
    ...
    func updateProgressMeter(with propgress: Progress) {
        delegate.updateProgressMeter(with: propgress)
    }
    
    func seekInputStream(offset: Int64, origin: CInt) -> CInt {
        NSURLSession.printDebug("[EasyHandle] -> seek callback \(offset) \(origin)")
        let d: CInt = {
        ...
        NSURLSession.printDebug("[EasyHandle] <- seek callback \(d)")
        return d
    }
}
...
extension NSURLSessionTask.EasyHandle {
    ...
    internal class CurlStringList {
        private var rawList: OpaquePointer? = nil
        init() {}
...
extension NSURLSessionTask.EasyHandle.CurlStringList {
    func append(_ string: String) {
        string.withCString {
            rawList = CFURLSessionSListAppend(rawList, $0)
        }
    }
    var asUnsafeMutablePointer: UnsafeMutablePointer<Void>? {
        return rawList.map{ UnsafeMutablePointer<Void>($0) }
    }
}
...
extension CFURLSessionEasyCode : ErrorProtocol {
