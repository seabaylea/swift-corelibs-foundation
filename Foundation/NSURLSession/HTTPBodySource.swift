...
private extension HTTPBodyFileSource {
    ...
    private func readNextChunk() {
        ...
        dispatch_io_read(channel, 0, lengthToRead, workQueue) { (done: Bool, data: dispatch_data_t?, errno: Int32) in
...
extension HTTPBodyFileSource : HTTPBodySource {
    func getNextChunk(withLength length: Int) -> HTTPBodySourceDataChunk { //fatalError("unimplemented") }   
        switch availableChunk {
        case .empty:
            readNextChunk()
            return .retryLater
